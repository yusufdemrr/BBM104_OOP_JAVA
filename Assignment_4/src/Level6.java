...
public class Level6 {
    ...
    public Level6() {
        ...
        Duck duck6 = new Duck();
        duck = duck6;
        duck.setDUCK_SPEED(10);
        duckView6 = duck6.getDuckView();
    }

    public ImageView getDuckView6() {
        return duckView6;
    }
    ...
    private void shootDuck() {
        ...
        duckView6.setImage(duck.duckShotting);

        // duckShotting resminin 1 saniye gösterilmesi için bir geçiş süresi tanımlanıyor
        TranslateTransition shottingTransition = new TranslateTransition(Duration.seconds(1), duckView6);
        shottingTransition.setByY(0);
        shottingTransition.play();

        shottingTransition.setOnFinished(event -> {
            duckView6.setImage(duck.duckFalling);
            TranslateTransition fallTransition = new TranslateTransition(Duration.seconds(1), duckView6);
            fallTransition.setByY(100/3*ViewManager.Scale); // 100 piksel aşağı düşme animasyonu
            fallTransition.setOnFinished(fallEvent -> {
                duckView6.setVisible(false); // Ördek resmini görünmez yap
                handleGameFinish();
    ...
    /**
     * Performs the necessary actions when the game is completed.
     * Creates a game finish label and adds it to the center of the screen.
     * Adds the game finish label to the game pane.
     * Sets up event listeners for the Enter and Escape keys to restart the game or exit the application.
     * If the Enter key is pressed, removes the game finish label, resets the game state, draws the initial level, and redraws the foreground and crosshair.
     * If the Escape key is pressed, stops the level completed effect, resets the game state, closes the game stage, and shows the main stage.
     */
    private void handleGameFinish() {
        // Oyun bittiğinde yapılacak işlemler
        Label gameFinishLabel = new Label("You Have Completed The Game!\nPress Enter to Play Again\nPress ESC to Exit");
     ...
     * Creates a game over label and adds it to the center of the screen.
     * Adds the game over label to the game pane.
     * Sets up event listeners for the Enter and Escape keys to restart the game or exit the application.
     * If the Enter key is pressed, removes the game over label, resets the game state, draws the initial level, and redraws the foreground and crosshair.
     * If the Escape key is pressed, stops the game over effect, resets the game state, closes the game stage, and shows the main stage.
     */
