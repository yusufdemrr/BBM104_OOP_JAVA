...
public class Level2 {
    ...
    public Level2() {
        ...
        Duck duck2 = new Duck();
        duck = duck2;
        duck.setDUCK_SPEED(5);
        duckView2 = duck2.getDuckView();
    }

    public ImageView getDuckView2() {
        return duckView2;
    }
    ...
    private void shootDuck() {
        ...
        duckView2.setImage(duck.duckShotting);

        // duckShotting resminin 1 saniye gösterilmesi için bir geçiş süresi tanımlanıyor
        TranslateTransition shottingTransition = new TranslateTransition(Duration.seconds(1), duckView2);
        shottingTransition.setByY(0);
        shottingTransition.play();

        shottingTransition.setOnFinished(event -> {
            duckView2.setImage(duck.duckFalling);
            TranslateTransition fallTransition = new TranslateTransition(Duration.seconds(1), duckView2);
            fallTransition.setByY(100/3*ViewManager.Scale); // 100 piksel aşağı düşme animasyonu
            fallTransition.setOnFinished(fallEvent -> {
                duckView2.setVisible(false); // Ördek resmini görünmez yap
                handleGameFinish();
    ...
    private void handleGameFinish() {
        ...
        GameViewManager.gameScene.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.ENTER) {
                ...
                GameViewManager.gamePane.getChildren().add(level3.getDuckView3());
