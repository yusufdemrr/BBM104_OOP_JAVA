...
public class Level5 {
    ...
    public Level5() {
        ...
        Duck duck5 = new Duck();
        duck = duck5;
        duck.setDUCK_SPEED(9);
        duckView5 = duck5.getDuckView();
    }

    public ImageView getDuckView5() {
        return duckView5;
    }
    ...
    private void shootDuck() {
        ...
        duckView5.setImage(duck.duckShotting);

        // duckShotting resminin 1 saniye gösterilmesi için bir geçiş süresi tanımlanıyor
        TranslateTransition shottingTransition = new TranslateTransition(Duration.seconds(1), duckView5);
        shottingTransition.setByY(0);
        shottingTransition.play();

        shottingTransition.setOnFinished(event -> {
            duckView5.setImage(duck.duckFalling);
            TranslateTransition fallTransition = new TranslateTransition(Duration.seconds(1), duckView5);
            fallTransition.setByY(100/3*ViewManager.Scale); // 100 piksel aşağı düşme animasyonu
            fallTransition.setOnFinished(fallEvent -> {
                duckView5.setVisible(false); // Ördek resmini görünmez yap
                handleGameFinish();
    ...
    private void handleGameFinish() {
        ...
        GameViewManager.gameScene.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.ENTER) {
                ...
                GameViewManager.gamePane.getChildren().add(level6.getDuckView6());
