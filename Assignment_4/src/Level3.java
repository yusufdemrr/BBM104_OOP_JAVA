...
public class Level3 {
    ...
    public Level3() {
        ...
        Duck duck3 = new Duck();
        duck = duck3;
        duck.setDUCK_SPEED(6);
        duckView3 = duck3.getDuckView();
    }

    public ImageView getDuckView3() {
        return duckView3;
    }
    ...
    private void shootDuck() {
        ...
        duckView3.setImage(duck.duckShotting);

        // duckShotting resminin 1 saniye gösterilmesi için bir geçiş süresi tanımlanıyor
        TranslateTransition shottingTransition = new TranslateTransition(Duration.seconds(1), duckView3);
        shottingTransition.setByY(0);
        shottingTransition.play();

        shottingTransition.setOnFinished(event -> {
            duckView3.setImage(duck.duckFalling);
            TranslateTransition fallTransition = new TranslateTransition(Duration.seconds(1), duckView3);
            fallTransition.setByY(100/3*ViewManager.Scale); // 100 piksel aşağı düşme animasyonu
            fallTransition.setOnFinished(fallEvent -> {
                duckView3.setVisible(false); // Ördek resmini görünmez yap
                handleGameFinish();
    ...
    private void handleGameFinish() {
        ...
        GameViewManager.gameScene.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.ENTER) {
                ...
                GameViewManager.gamePane.getChildren().add(level4.getDuckView4());
